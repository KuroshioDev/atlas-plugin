...
const { pluginRoot,pluginResources, pluginName } = require( "../model/path.js")
const Reply = require( "../model/reply.js")
const { library, list } = require( "../model/moreLib.js")
        ...
        if (!await this.index(sync, Tmpmsg, rule, libpath)) {
          ...
          if (rightname in imagePath[sync]) {
            ...
            if (fs.existsSync(path)) {
              // 回复图片
              this.e.reply(segment.image('file:///'+path))
              this.islog = true
              // 是否交给其他插件处理
      ...
      if (key === element) {
        ...
        if (!result || !result.message_id) {
          let base64 = await puppeteer.screenshot('AtlasIndex', {
            tplFile: `plugins/${pluginName}/resource/massage/text.html`,
            pluResPath: `${pluginResources}/`,
            imgType: 'png',
            massage: sendmsg,
            name: this.e.nickname
          })
          result = await this.e.reply(base64)
        }
